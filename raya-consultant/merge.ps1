...
$installmentRegex = '(?s)(<div class="subsection">\s*<div class="subsection-title">برنامج الأفراد \(العميل الأساسي\).*?</div>\s*</div>\s*</div>)'
if ($html -match $installmentRegex) {
    $installmentContent = $matches[1]
    $lastIndex = $installmentContent.LastIndexOf("</div>")
    if ($lastIndex -ge 0) {
        $installmentContent = $installmentContent.Substring(0, $lastIndex).Trim()
    }
} else { $installmentContent = "" }

$salesRegex = '(?s)(<div class="subsection">\s*<div class="subsection-title">بروتوكول إتمام البيع – الخطوات الرسمية.*?</div>\s*</div>)'
if ($html -match $salesRegex) { $salesContent = $matches[1] } else { $salesContent = "" }

$fraudRegex = '(?s)(<div class="subsection">\s*<div class="subsection-title">بروتوكول حارس البيت – مكافحة الاحتيال.*?</div>\s*</div>)'
if ($html -match $fraudRegex) { $fraudContent = $matches[1] } else { $fraudContent = "" }

$dictRegex = '(?s)(<div class="subsection">\s*<div class="subsection-title">قاموس لغة صاحب البيت.*?</table>\s*</div>)'
if ($html -match $dictRegex) { $dictContent = $matches[1] } else { $dictContent = "" }
...
$newBlock = $template.Replace("{{SALES}}", $salesContent).Replace("{{FRAUD}}", $fraudContent).Replace("{{DICT}}", $dictContent).Replace("{{INSTALLMENT}}", $installmentContent)

$fullRegex = '(?s)<!-- ── SECTION: INSTALLMENT ── -->\s*<div class="section-block" data-category="installment" id="sec-installment">.*?<!-- ── SECTION: PROTOCOLS ── -->\s*<div class="section-block" data-category="protocols" id="sec-protocols">.*?</table>\s*</div>\s*</div>\s*</div>'

$newHtml = $html -replace $fullRegex, $newBlock

if ($newHtml -ne $html) {
    Set-Content -Path "index.html" -Value $newHtml -Encoding UTF8
    Write-Output "Successfully merged and updated Protocols section."
} else {
    Write-Output "Failed to match the replacement block."
}
