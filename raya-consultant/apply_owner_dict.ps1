$lines = Get-Content 'index.html' -Raw -Encoding UTF8
$linesArr = $lines -split "
"
$outLines = @()

for ($i=0; $i -lt $linesArr.Count; $i++) {
    if ($linesArr[$i] -match "onclick="filterSection\('house_guard', this\)"") {
        $outLines += $linesArr[$i]
        $outLines += '      <button class="toc-chip" onclick="filterSection('owner_dictionary', this)">قاموس صاحب البيت</button>'
    } elseif ($linesArr[$i] -match "</div><!-- end manualContent -->") {
        $outLines += @'
...
Write-Output "Applied owner_dictionary section successfully."
