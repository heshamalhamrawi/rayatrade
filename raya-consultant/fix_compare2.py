...
def fix_compare():
    ...
    new_func = """window.runSmartCompare = function() {
  ...
  sortedKeys.forEach(k => {
    ...
    let isSpec1Avail = v1 && v1 !== '-';
    let isSpec2Avail = v2 && v2 !== '-';
    ...
    let cell1 = v1;
    if (v1 === '-') {
        cell1 = '<span style="color:#CBD5E1;">-</span>';
    } else if (highlight1) {
    ...
    let cell2 = v2;
    if (v2 === '-') {
        cell2 = '<span style="color:#CBD5E1;">-</span>';
    } else if (highlight2) {
...
"""
    ...
    pattern = re.compile(r'window\.runSmartCompare\s*=\s*function\(\)\s*\{.*?\n\};\n?', re.DOTALL)
    html = pattern.sub(new_func + "\n", html)
